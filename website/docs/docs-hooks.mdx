...
This hook provides you a set of navigation helpers that aims to reduce the boilerplate required to execute common navigation commands specially those that need the `componentId` argument.

:::tip Use Navigation Context
In order to use this hook your screens must use the [NavigationProvider](contexts#navigationprovider) to pass the **componentId** to the three below so the **useNavigation** hook can read it.

You can take advantage of the [withNavigationProvider](hocs#withnavigationproviders) or you could also use the [NavigationProvider](contexts#navigationprovider) manually.
:::

Then in your component just need to import **useNavigation** to obtain the navigation commands helpers contextualized with the **componentId** out of the box 💪

#### 💅 &#12644;**This is how it looks like**
...
You can check the list of the [supported methods](helpers#createnavigationcommands) and the arguments overload.
